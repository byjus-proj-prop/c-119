...
for word_tags in word_tags_list:
    ...
    for word in stem_words:
        if word in pattern_words: bag_of_words.append(1);
...
def preprocess(training_data0):
    training_data0 = np.array(training_data0, dtype=object)
    train_x = list(training_data0[:,0]);
    train_y = list(training_data0[:,0]);
    return train_x, train_y

train_x, train_y = preprocess(training_data);
